...
async function runXScript(
  scriptName: string,
  args: object,
): Promise<XScriptResult> {
...
export async function processTaskIpc(
  ...
  if (data.type?.startsWith('x_')) {
    ...
    if (!requestId) {
      logger.warn(
        { type: data.type },
        'X integration blocked: missing requestId',
      );
      return;
    }
