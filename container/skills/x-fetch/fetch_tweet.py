...
def main():
    ...
    # ── Backend selection (moved to top) ──────────────────────────────────
    ...
    if args.search:
        ...
        if use_nitter:
            ...
            result = {"query": args.search, "tweets": tweets, "count": len(tweets), "backend": "nitter"}
        else:
            # Fallback to browser search (via Google/DDG)
            try:
                # Ensure we search only site:x.com for Twitter results
                search_query = args.search
                if "site:x.com" not in search_query:
                     search_query = f"site:x.com {search_query}"
                
                # Top-level import check to see if playwright_client is loaded
                if not check_camofox(args.port):
                     raise ImportError("Browser backend not running")
                
                # camofox_search returns list of {"title", "url", "snippet"}
                from playwright_client import camofox_search
                raw_results = camofox_search(search_query, num=args.limit, port=args.port)
                ...
                result = {"query": args.search, "tweets": tweets, "count": len(tweets), "backend": "browser"}
            except Exception as e:
                result = {"query": args.search, "tweets": [], "count": 0, "error": str(e)}

        if args.text_only:
            print(f"搜索 \"{args.search}\" — {len(tweets)} 条结果\n")
            for i, tw in enumerate(tweets, 1):
                print(f"[{i}] {tw.get('author_name', tw.get('display_name',''))} ({tw.get('author', tw.get('username',''))}) · {tw.get('time_ago', tw.get('time',''))}")
